...
receiver's email address, email subject, and message content as input parameters.'''
    ...
    server.login(sender,"xxxx xxxx xxxx xxxx")               # dummy passcode. Sender should be your email id. passcode is app password. Explained in detail in readme file
...
administrator to approve or reject the mentor's profile.'''
    ...
    if viewer=='mentee':
        if mentee_id>0:
            ...
            return render_template('profile_mentor.html',mentor=mentor,msg='',view_profile=True,give_approval=False,viewer=viewer,mentee={})
...
def view_mentor_complaint():
    ...
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute(
            'SELECT * FROM mentor_complaints where complaint_id = %s',(complaint_id,))
    complaint=cursor.fetchone()
    cursor.execute(
...
def submit_feedback():
    ...
    if request.method == 'POST':
        ...
        cursor.execute('SELECT * FROM mentee WHERE username=%s',(username,))
        mentee=cursor.fetchone()
        send_email(mentify_email,mentee['email_id'],"Feedback submitted successfully",f"Dear {mentee['mentee_name']},your feedback has been submitted. Thank you for submitting your valuable feedback.\nLogin to our website to continue learning!!.")
        return redirect(url_for('dashboard_mentee',username=username))
